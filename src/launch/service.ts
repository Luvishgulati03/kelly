 ...
 *   run(slug) — after the owner fills in ANSWER: blanks, fans out gtm-strategist (t2),
...
export class LaunchCrewService {
  ...
  async intake(input: string): Promise<LaunchIntakeResult> {
    ...
    const prompt = [
      ...
      `Derive the exact list of datapoints ${this.config.ownerName} must answer before a GTM strategist can build a launch roadmap.`,
  ...
  async run(slug: string): Promise<LaunchRunResult> {
    ...
    const gtmPromise = (async (): Promise<CrewFinding> => {
      ...
      const prompt = [
        ...
        "", `--- ${this.config.ownerName}'s answers ---`, qaBlock,
