...
/**
 * Statuses worth buzzing the owner's phone about (their rule, 2026-08-15): "don't remind me for
...
async function writeTrackerState(config: HenryConfig, state: TrackerState): Promise<void> {
  ...
  // The .md is the artifact the owner actually reads — no restrictive mode, same as cover letters / linkedin drafts.
...
export async function recordTrackerEvents(config: HenryConfig, input: StructuredTrackerEvent[], options: { backfill?: boolean } = {}): Promise<TrackerUpdateResult> {
  ...
  for (const app of parsed) {
    ...
    if (!advances) {
      ...
      // and never ping the owner about old news.
