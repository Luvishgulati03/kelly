 ...
 * (granted by the owner, 2026-08-15). Every clause of that exception is a coded gate here,
 ...
 *   "logged and mirrored to the owner" → activity record + Telegram mirror on every post.
 ...
 * data/social/staged/<date>.md and tells the owner why. Staging never consumes the day's post.
 */
...
export interface TweetRunOptions {
  /** `cron` obeys the kill switch as a silent no-op and the daily jitter slot; `cli` is the owner asking now. */
...
/** Local "YYYY-MM-DD" — the once-per-day rail lives in the owner's local day, not UTC. */
...
export class TweetService {
   ...
   * provider call, because a disabled feature that still burns the owner's quota is a bug.
   */
  async run(options: TweetRunOptions = {}): Promise<TweetRunResult> {
    ...
    // and stages (the owner asked for it in person); the unattended cron simply does nothing.
    ...
    try {
      ...
      try {
        ...
        // A failed post never touches the ledger: today stays retryable, and the owner still sees the text.
  ...
  /**
   * ONE provider call in the owner's voice, grounded on personality.md exactly like the LinkedIn
  ...
  private prompt(topic: TweetTopic, persona: string, rejected?: { text: string; reason: string }): string {
    return [
      `Write ONE tweet in ${this.config.ownerName}'s voice about the technology development below. ${this.config.ownerName} is a product-minded software engineer; the tweet should read like a builder noticing something funny about the industry, not like a news bot or a hype account.`,
      `Hard rules (a violation gets the draft thrown away): at most ${TWEET_MAX_CHARS} characters. Humorous, but NEVER punching down — the joke lands on the situation, the hype, or Henry/${this.config.ownerName} himself, never on a person, a company's employees, or a group. No politics, no controversy, no named individuals, no employers. Zero @mentions of anyone. Zero or one hashtag, never more. Emojis only if exactly one genuinely lands. It must clearly reference the actual development below, specifically enough that a reader knows what happened.`,
   ...
   * data/social/staged/<date>.md and the owner is told WHY it did not go out. Staging never
  ...
  private async stage(store: TweetStore, date: string, topic: TweetTopic, text: string, reason: string, now: Date, trigger: TweetTrigger): Promise<TweetRunResult> {
    ...
    // A hand-run `henry tweet [draft]` must never suppress the automated post the owner asked for.
