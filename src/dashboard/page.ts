...
var isHindi=function(t){return /[\\u0900-\\u097f]/.test(t||'')};
...
/* ---- overview ---- */
var latestUsage=null,latestVoice=null,latestApprovals=[];
...
function refreshOverview(){return Promise.all([get('/api/usage').catch(function(){return null}),get('/api/voice/transcripts?limit=6').catch(function(){return null}),get('/api/approvals').catch(function(){return[]})]).then(function(r){latestUsage=r[0];latestVoice=r[1];latestApprovals=r[2]||[];
...
function langChip(t){var l=t.language||'';if(!t.text)return'<span class="lang">–</span>';var hi=isHindi(t.text);return hi?'<span class="lang hi">हिं</span>':l==='en'?'<span class="lang">EN</span>':'<span class="lang">Hinglish</span>'}
function transcriptRow(t,sel){var who=t.surface==='telegram'?'<div class="who tg">TG</div>':'<div class="who ctr">CTR</div>';var text=t.text||(t.error?'('+t.error+')':'(no words)');var ents=[].concat(t.entities.brands,t.entities.quantities.slice(0,2)).join(' · ');return'<div class="item'+(sel?' sel':'')+'" data-id="'+esc(t.id)+'" tabindex="0">'+who+'<div><div class="t'+(isHindi(text)?' hi':'')+'">'+esc(text)+'</div><div class="m">'+langChip(t)+'<span>'+fmtWhen(t.at)+'</span>'+(t.durationSeconds?'<span>'+fmtSec(t.durationSeconds)+'</span>':'')+(t.sttMs?'<span>stt '+fmtDur(t.sttMs)+'</span>':'')+(ents?'<span>'+esc(ents)+'</span>':'')+'</div></div>'+stateChip(t.state)+'</div>'}
