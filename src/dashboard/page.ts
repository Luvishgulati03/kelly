...
<div class="wrap"><div class="shell">
  <nav class="rail" aria-label="Sections">
    ...
    <div class="tabs" role="tablist">
      ...
      <a class="tab" href="/talk" id="openTalk">Talk ↗</a>
  ...
  <div class="pane" id="pane-voice">
    ...
    <div class="card" style="margin-bottom:12px"><h3>Counter mode</h3>
      <div class="settings">
        <label>Counter mode: <select class="input" id="set-counter-mode"><option value="review">review</option><option value="conversation">conversation</option><option value="talk">talk</option></select></label>
        <div style="grid-column:1/-1;font-size:12px;color:var(--ink-3)">
          review: a transcript is shown and typed "send" confirms it before it reaches Kelly.<br>
          conversation: transcripts go straight to Kelly and replies are spoken back, still tap-to-talk.<br>
          talk: hands-free — Kelly greets, listens, replies, and listens again with no tap between turns.
        </div>
        <div><button class="button primary" id="set-counter-mode-save" type="button">Save counter mode</button> <span class="mono" id="set-counter-mode-note" style="font-size:12px;color:var(--ink-3)"></span></div>
      </div>
    </div>
...
function langChip(t){if(!t.text)return'<span class="lang">–</span>';var hi=/[ऀ-ॿ]/.test(t.text),latin=/[A-Za-z]/.test(t.text);return'<span class="lang">'+(hi&&latin?'Mixed':hi?'Hindi':'English')+'</span>'}
...
ids('set-counter-mode-save').addEventListener('click',function(){ids('set-counter-mode-note').textContent='saving…';post('/api/voice/settings',{counterMode:ids('set-counter-mode').value}).then(function(r){ids('set-counter-mode-note').textContent=r.error?r.error:'saved';loadTranscripts()}).catch(function(e){ids('set-counter-mode-note').textContent=e.message})});
