...
export class WorkflowScheduler {
  ...
  private async runStandup(kind: "standup.prompt" | "standup.scan" | "standup.summary", session: "morning" | "evening"): Promise<unknown> {
    ...
    try {
      ...
      // Courtesy yield: if the owner is mid-conversation in another process, let their turn
  ...
  /**
   * Daily portfolio stats refresh + deploy (the owner's standing authorization,
   ...
   * awaiting the owner's go → skip, because this workflow must never be what first
  ...
  private async runPortfolioStats(): Promise<unknown> {
    ...
    if (ahead > 0) return { skipped: true, reason: `local main is ${ahead} commit(s) ahead of origin — unshipped changes await ${this.config.ownerName}'s go` };

    // GitHub contribution graph: refetch real data (best-effort — offline keeps
    // yesterday's bake) and re-bake the static markup, so the owner's commits appear
   ...
   * cron path never passes a prepare count: staging drafts is an owner-invoked CLI act.
   */
