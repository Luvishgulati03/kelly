 ...
 * tailored to it PLUS a cover letter, in one flow. Non-negotiable contract, from the owner:
 *
 ...
 * 1. The PDF is rendered from a fixed HTML template that replicates the owner's real resume
...
/** Parse the owner's resume.md (stable known format) into the template's data model. */
...
/**
 * Fixed template replicating the owner's real resume PDF (Word-blue, Letter). Content-only slots.
...
export class TailoredApplicationService {
  ...
  private tailorPrompt(base: ResumeData, baseMd: string, jd: string, feedback?: string): string {
    return [
      `You tailor ${this.config.ownerName}'s resume CONTENT to a job description. Formatting, structure, employers, titles, dates, education, and section order are LOCKED — you only reword and reorder text.`,
      ...
      `- profile: 2-3 sentences. FIRST sentence leads with the JD's literal role title + ${this.config.ownerName}'s top 1-2 genuinely-shared qualifiers (the summary carries the heaviest ATS keyword weight); include his single strongest quantified achievement; mirror JD phrasing naturally, never as a stapled keyword list.`,
      ...
      `- changes: 3-6 short lines describing what you emphasized and why (for ${this.config.ownerName}'s review).`,
  ...
  async run(jdText: string): Promise<{ dir: string; resumePdf: string; coverPdf: string; company: string; role: string; changes: string[] }> {
    ...
    // changes.md was written before the render — the durable artifact the owner
