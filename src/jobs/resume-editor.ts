...
export class ResumeEditorService {
  ...
  /** Rewrites the resume per instructions. Never overwrites resume.md — returns a new draft for the owner to review. */
  async edit(instructions: string): Promise<{ markdownPath: string; pdfPath: string }> {
    const resume = await this.resume();
    const prompt = [
      `Rewrite ${this.config.ownerName}'s resume markdown per the instructions below. STRICT rules: never invent employers, roles, dates, degrees, metrics, or skills not present in the current resume or the instructions; reordering, rewording, emphasis changes, and cutting are allowed; additions come ONLY from the instructions. Keep it one page dense. ${this.config.ownerName}'s target: Product Manager roles. Return ONLY the full updated resume as markdown.`,
      `\n--- current resume ---\n${resume}`,
      `\n--- instructions ---\n${instructions}`,
    ].join("\n");
  ...
  /** The owner's acceptance step: copies an edited draft over the canonical resume source. */
