...
/**
 * Morning job scout: one daily pass over NAUKRI (riding the owner's already-logged-in
...
/** The one line the owner gets when their Naukri session has lapsed — the whole manual fix, inside the message. */
...
export interface ScoutBrowser {
  ...
  /** HEADED session grant: Naukri + X login tabs; resolves when the owner closes the window. */
  login(): Promise<void>;
...
export interface ScoutShortlistEntry extends ScoutVerdict {
  ...
  location: string;
  postedAge: string;
  /** Where the row came from (naukri / web / …) — printed in the report so the owner knows what they are looking at. */
...
/** Local "YYYY-MM-DD" — the once-per-day guard lives in the owner's local day, not UTC. */
  ...
  async importLinkedInCookie(liAt: string): Promise<{ ok: boolean; reason?: string }> {
    ...
    try {
      ...
      await this.activity.record(out ? "workflow.failed" : "workflow.completed",
        out ? "LinkedIn cookie import failed verification (still logged out)" : `LinkedIn session imported via cookie — the opt-in linkedin source can search as ${this.config.ownerName}`,
        { scout: true });
...
export class JobScoutService {
  ...
  /** One-time session grant: the owner logs in to Naukri (+ X) in a headed window; the persistent profile keeps both. */
  async login(): Promise<void> {
  ...
  async scout(options: { prepare?: number; now?: Date } = {}): Promise<ScoutResult> {
    ...
    // Titles source chain: the alert profile the owner curated themself (jobs alerts-sync)
  ...
  /**
   * ONE batched t1 call scoring every new listing against the owner's real profile.
  ...
  private async score(fresh: ScoutListing[], leads: ScoutLead[]): Promise<ScoutVerdict[]> {
    ...
    const prompt = [
      ...
      `Score how well each listing fits ${this.config.ownerName}, grounded ONLY in his resume and application profile below.`,
      ...
      `\n--- ${this.config.ownerName}'s resume (${this.config.resumeSourcePath}) ---\n${resume || "No resume file found."}`,
  ...
  /** The ranked artifact the owner actually reads — data/scout/<date>.md. */
