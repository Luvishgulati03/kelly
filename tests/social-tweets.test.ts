...
test("kill switch OFF: the cron job is a pure no-op — no provider call, no poster", async () => {
  ...
  assert.equal(runner.calls(), 0, "a disabled feature must not spend the owner's provider quota");
...
test("kill switch OFF: a hand-run `henry tweet` stages the draft and never posts", async () => {
  ...
  assert.equal(notes.length, 1, "the owner is told on Telegram why it did not go out");
  assert.match(notes[0], /staged, NOT posted/);
});

test("enabled + keys: posts once, logs it, mirrors it to the owner, and the day is then spent", async () => {
  const config = tempConfig();
  enable(config, true);
...
test("two off-policy drafts in a row stage instead of posting — the retry is not a bypass", async () => {
  ...
  assert.ok(result.stagedPath, "the owner still sees what it wanted to say");
});
...
test("flipping the switch OFF mid-draft still stops the post — OFF wins instantly", async () => {
  ...
  enable(config, true);
  const poster = new FakePoster();
  // The "provider turn" is where the owner reaches for the kill switch.
  const runner = {
    run: async () => {
      enable(config, false);
...
test("a failed post leaves the day retryable and tells the owner what happened", async () => {
  const config = tempConfig();
  enable(config, true);
  const notes: string[] = [];
